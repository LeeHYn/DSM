...
| Task 8 selective foundation classification | Four commits and ten capability rows classified with evidence/cost; only the partial active-schedule index conditionally `PORT` to Task 9 | PASS |
...
| Daily finalization/automatic snapshots | Foundation runs UTC 00:05 recompute/snapshot. Current has only actor-triggered snapshot and score code still counts any `COMPLETED` Task; approved F-006 requires same-day non-null `completedAt`. No safe exact sequence exists here. | `DEFER`. Wrong defer leaves finalization manual; wrong port automatically persists an incorrect score/ranking projection and cron side effect. |
